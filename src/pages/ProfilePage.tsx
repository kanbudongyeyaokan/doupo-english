...
export function ProfilePage({ canInstall, installed, onInstall }: ProfilePageProps) {
  ...
  const runImport = async (mode: 'merge' | 'replace') => {
    ...
    try {
      ...
      const isSourceRefresh = pending.data.format === 'doupo-english-vocabulary'
        && pending.data.batch?.updateStrategy === 'source-authoritative'
      const replacementSuffix = result.replacedWords ? `，并归并 ${result.replacedWords} 个旧词条` : ''
      setMessage(result.unchanged
        ? `数据未变化，已跳过 ${pending.preview.unchanged} 个重复单词`
        : `已${isSourceRefresh ? '校订更新' : mode === 'merge' ? '合并导入' : '覆盖恢复'} ${pending.preview.incoming} 个单词${replacementSuffix}`)
      setPending(null)
    } catch (error) {
