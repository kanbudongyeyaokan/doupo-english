  ...
  const runImport = async (mode: 'merge' | 'replace') => {
    ...
    try {
      ...
      setMessage(result.unchanged
        ...
        : `已${isSourceRefresh ? '校订更新' : mode === 'merge' ? '合并导入' : '覆盖恢复'} ${pending.preview.incoming} 个单词`)
      setPending(null)
    } catch (error) {
