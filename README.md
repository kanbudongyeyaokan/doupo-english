...
- 必考词 Unit 7 的第 61–72 词人工校订包位于 `private-materials/imports/redbook-27-required-unit-07-61-72.json`。顺序为 `wage`、`wander`、`want`、`way`、`weak`、`wear`、`weary`、`weigh`、`weight`、`weird`、`welfare`、`whirl`；内容已逐项对照 PDF 书页 53–54，恢复 `wage/salary/pay` 辨析、`way` 的五个义项和九组词组、`weary`、`weight`、`whirl` 的多词性与完整例句，并纠正 `wear` 例句粘连及 `weird`、`welfare` 音标串栏。该包已通过 12 个稳定 ID、连续顺序、重复导入不增词，以及 FSRS、复习记录、笔记、标签、收藏、错词状态和本地二进制图片保留验证；已在在线 PWA 中合并导入并经刷新确认，Unit 7 总数仍为 80，完整词库仍为 3,858。
- 必考词 Unit 7 的第 73–80 词人工校订包位于 `private-materials/imports/redbook-27-required-unit-07-73-80.json`。顺序为 `key`、`knit`、`whisper`、`yield`、`youth`、`zigzag`、`zone`、`zoom`；内容已逐项对照 PDF 书页 55，恢复 `key` 的名词／形容词／动词、`whisper`、`yield`、`zigzag`、`zone`、`zoom` 的多词性与完整例句，并保留 `yield` 的 2014 年考研阅读句。页底罗斯福名言属于版面附录，没有错误归入相邻词条。该包已通过 8 个稳定 ID、连续顺序、重复导入不增词，以及 FSRS、复习记录、笔记、标签、收藏、错词状态和本地二进制图片保留验证。
- 必考词 Unit 7 已完成全部 80 词人工校订。可一次在手机端合并导入的整单元包位于 `private-materials/imports/redbook-27-required-unit-07-curated.json`，词序从 `take` 到 `zoom`；该包已通过 80 个稳定 ID、整单元连续顺序、重复导入去重和学习状态保留验证，并已在在线 PWA 中合并导入、刷新后核对词数和末词字段。
- 必考词 Unit 8 的首组 12 词人工校订包位于 `private-materials/imports/redbook-27-required-unit-08-first-12.json`。顺序为 `aggravate`、`aggregate`、`aggressive`、`agitate`、`agreeable`、`aid`、`aim`、`brief`、`brisk`、`browse`、`diffuse`、`emphasis`；内容已逐项对照 PDF 书页 56–57、60，纠正整组统一错标为书页 56 的 OCR 页码，恢复 `aggregate` 三种词性、`aggressive` 跨页的 2016 年考研阅读句、`agreeable/pleasant` 辨析、`brief` 典型考题及 `diffuse` 双读音和 2016 年考研阅读句。该包已通过稳定 ID、连续顺序、重复导入不增词，以及 FSRS、复习记录、笔记、标签、收藏、错词和本地图片保留验证；已在在线 PWA 中合并导入并经刷新确认，Unit 8 仍为 70 词，完整词库仍为 3,858。
